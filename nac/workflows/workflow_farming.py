...
from scheduleCp2k import (prepare_job_cp2k, prepare_cp2k_settings,
                          prepare_farming_cp2k_settings)
...
def main():
    ...
    cp2k_args.cell_parameters = [[16.11886919, 0.07814137, -0.697284243],
                                 [-0.215317662, 4.389405268, 1.408951791],
                                 [-0.216126961, 1.732808365, 9.748961085]]
    cp2k_args.specific.cp2k.force_eval.dft.scf.eps_scf = 1e-5
    ...
    calculate_pyxaid_hams('cp2k', project_name, geometries, cp2k_args)
    plams.finish()
...
def calculate_mos(package_name, all_geometries, work_dir, path_hdf5, folders,
    ...
    def prepare_jobs(job_files, start_index=0):
        ...
        # The first Job does a previous calculation and then read from its
        # own restart. The second Job read from the first and so on.
        jobs = []
        restart = join(work_dirs[0], 'point_0-RESTART.wfn')
        for gs, fs, wd, k in zip(gss, files, work_dirs, indexes):
          
            s = prepare_cp2k_settings(gs, fs, package_args, k, wd,
                                      hdf5_file=path_hdf5,
                                      wfn_restart_job=restart,
                                      farming_use_guess=True)
            jobs.append(s)
    ...
    if all(x is not None for x in paths_to_prop):
        ...
        # calculate initial guess for the farming job
        gs_0 = all_geometries[0]
        fs_0 = jobs_files[0]
        wd_0 = folders[0]
        initial_guess = prepare_job_cp2k(gs_0, fs_0, package_args, 0, wd_0,
        ...
        r = cp2k_farming(farming_settings, None, hdf5_file=path_hdf5,
                         ...
                         jobs_to_harvest=jobs, initial_guess=initial_guess)
        return r.orbitals
