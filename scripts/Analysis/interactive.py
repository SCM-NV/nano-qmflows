def ask_question(q_str, special='None', default=None):
    ...
    while True:
        question = str(input(q_str)) if sys.version_info[0] == 3 else str(raw_input(q_str))
        funcs = {'None': [str, {}], 'bool': [bool, {'y': True, 'yes': True, 'n': False, 'no': False}], 'int': [int, {}], 'float': [float, {}] }
