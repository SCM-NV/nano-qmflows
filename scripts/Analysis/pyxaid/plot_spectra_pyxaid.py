...
def read_energies(path, fn, s1, s2, nconds, ts):
    ...
    cols = (s1*2+5,s2*2+5) # index of columns in the me_energy file 
...
def main(path_output, s1, s2, ts, nconds, wsd, wdeph):
    ...
    acf = np.stack(np.stack(autocorrelate(en_states[i, :, j]) for i in range(en_states.shape[0])) for j in range(nconds)).transpose()
    ...
    sd = np.stack(spectral_density(acf_av[:, 1, i]) for i in range(en_states.shape[0])).transpose()
...
if __name__ == "__main__":
    ...
    msg = " plot_decho -p <path/to/hamiltonians> -s1 <State 1> -s2 <State 2> -ts <time window for analysis> -nconds <number of initial conditions> -wsd <energy window for spectral density plot in cm-1> -wdeph <time window for dephasing time in fs>"
    ...
    parser.add_argument('-nconds', required=True, type=int, help='Number of initial conditions')
