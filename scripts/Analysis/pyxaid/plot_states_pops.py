...
It average the populations of each state upon several initial conditions and also allows to define macrostates.
A macrostate is formed by a group of (micro)states indexed according to pyxaid numbering. 
For example, if you have donor-acceptor system, you can group all (micro)states formed by excited states localized within the donor/acceptor and or charge transfer states into a few macrostates.  
This helps the analysis of the population traces when a large number of excited states is included in the NAMD simulation.

Example:

 plot_states_pops.py -p . -ms "[ [0], [1,3], [4,6] ]" -nconds 6 

This means that the path with the output files is the current path '.' . The number of initial conditions over which the populations are averaged are 6. 
The macrostates are defined as a list of lists. The [0] is the ground state, [1,3] form a macrostate of states 1 and 3 indexed as in pyxaid, [4,6] forms a macrostates of states 4 and 6.
"""
...
def read_populations(path, fn, nconds, ms):
    ...
    xs = [np.stack(np.loadtxt('{}{}'.format(inpfile,j), usecols=col) for j in range(nconds)) for col in cols]
    return xs 
...
def main(path_output, ms, nconds):
    ...
    outs_avg, pops_avg = [np.average(out, axis=0) for out in outs], [np.average(pop, axis=0) for pop in pops]
...
if __name__ == "__main__":
    msg = " plot_states_pops -p <path/to/output> -ms <list of microstates state indexes> -nconds <number of initial conditions>"

    parser = argparse.ArgumentParser(description=msg)
    parser.add_argument('-p', required=True, help='path to the Hamiltonian files in Pyxaid format')
    parser.add_argument('-ms', type=str, required=True, help='Macrostate defined as a list of microstates')
