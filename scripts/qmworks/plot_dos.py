...
def readatom(filename):
    with open(filename, 'r') as f:
         atom = f.readline().split()[6] # In the first line in column 6, the atom is defined 
    return atom 
...
def plot_stuff(ys, energies, legends, emin, emax): 
    colors = ['black', 'orange', 'blue', 'red', 'green', 'magenta', 'cyan', 'black', 'yellow'] # In case you need more colors, this list should be expanded. 
#   First plot
    left = 0 
...
#   Prepare the cumulative density of states
    ...
    dos = np.stack(convolute(energies, ys[:, i], x_points, sigma) for i in range(ys.shape[1])).T # Take transpose to have it in (x_points, # dos) format
    cum_dos = np.stack(np.sum(dos[:, 0:i+1], axis=1) for i in range(dos.shape[1])).T # Cumulate dos for all atoms/ligands
#   Plotting now 
    plt.figure(2)
    ref = np.zeros(x_points.size) 
    cum_dos = np.column_stack((ref, cum_dos)) # Add a zero line for filling colors
    plt.xlim(emin, emax)  
...
def main(group, emin, emax): 
    files = sorted(glob.glob('*-k*.pdos')) # Check files with PDOS
    legends = [readatom(files[i]) for i in range(len(files))] # Define the atom type for each DOS file  
    print(files, legends)  
    energies = energies = np.loadtxt(files[0], usecols=1) # MO energies
    energies *= 27.211 # Convert energies to eV

    xs = [np.loadtxt(files[i]) for i in range(len(files))] # Read Files with PDOS info
    ys = np.stack(np.sum(xs[i][:,3:], axis=1) for i in range(len(files))).transpose() # Add up all orbitals contribution for each atom type 
